...
parser = argparse.ArgumentParser(
    description='COVID-19 country growth visualization')
...
def TG_formula(TG, A):
    ...
    return Formula(lambda t: (A / TG) * ((t / TG)**6.23) / np.exp(t / TG),
                   text)


countries = [
    Country('Slovakia', Formula(lambda t: 8 * t**1.28, r'$8 \cdot t^{1.28}$'),
            10),
...
class CountryData:
    def __init__(self, path, country_basic):
        ...
        self.cumulative_active = list(
            filter(lambda x: x >= country_basic.case_count,
                   itertools.accumulate(self.active)))
        self.date_list = self.date_list[len(self.active) -
                                        len(self.cumulative_active):]
...
if args.country != "ALL":
    ...
    graphs = [
        dcc.Graph(id=country_data.name,
                  figure=country_data.create_country_figure())
