                                    ...
                                    for (let key in selectionContext.selections) {
                                        if (dataContext.hasOwnProperty(key)) {
                                            ...
                                            data.series.forEach((serie, si) => {
                                                const tmp = serie.date_list.map((x, index) => {
                                                    const y = serie.values[index];
                                                    if (relative) {
                                                        return [index + 1, y];
                                                    }
                                                ...
                                                if (serie.type === 'prediction') {
                                                    let value = Date.parse(serie.max_value_date);
                                                    if (relative) {
                                                        value = serie.date_list.indexOf(serie.max_value_date) + 1;
                                                    }
                                            ...
                                            if (data.hasOwnProperty('prediction_at_date')) {
                                                if (relative) {
                                                    predictionEnds.push(xValues.indexOf(data.prediction_at_date) + 1);
                                                } else {
                                                    predictionEnds.push(Date.parse(data.prediction_at_date));
                                                }
                                            }
                                        }
                                    }
