...
def test_two_curves():
    ...
    report = CountryReport("UK", "United Kingdom", dates, None, None, None, None, cumulative_active)
    ...
    assert abs(curve1.y_max - (47 / 2) * (max_t1 / 2) ** 1.5 * math.exp(-max_t1 / 2)) < 1e-6
    ...
    assert abs(curve2.y_max - (12 / 3) * (max_t2 / 3) ** 2 * math.exp(-max_t2 / 3)) < 1e-6
