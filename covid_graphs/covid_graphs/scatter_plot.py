...
class SimulationGraph:
    def __init__(self, country_data_file: Path, simulation_pb2_file: Path):
        ...
        print(
            f"b_0={self.best_b0}, {self.param_name}={self.best_param} is the best fit "
    ...
    def create_scatter_plot(self, graph_type=GraphType.Cumulative):
        ...
        title_text = (
            title_text.replace("_b0", f"{self.best_b0}")
            .replace("_alpha", f"{self.best_param}")
            .replace("_param", self.param_name)
            .replace("_prefix", f"{self.prefix_length}")
        )
